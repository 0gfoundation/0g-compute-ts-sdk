...
function MobileProviderCard({
    ...
    const isTeeVerified =
        provider.verifiability?.toLowerCase().includes('teeml') ||
        provider.verifiability?.toLowerCase().includes('tee')
    const isOfficial = OFFICIAL_PROVIDERS.some(
        (op) => op.address === provider.address
    )

    // Get real-time health status for this specific model
    ...
    const statusText = getHealthStatusText(healthStatus.status)
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-1 sm:space-x-2">
                                <span className="font-medium text-xs sm:text-sm truncate flex items-center gap-1.5">
                                    ...
                                    {isLoadingHealth ? (
                                        <span className="w-2 h-2 rounded-full flex-shrink-0 bg-gray-300 animate-pulse" />
                                    ) : (() => {
                                        const healthInfo = getHealthInfo(selectedProvider)
                                        return (
                                            <span
                                                className={`w-2 h-2 rounded-full flex-shrink-0 ${healthInfo.statusColor}`}
                                                title={`${healthInfo.statusText}${
                                                    healthInfo.healthStatus.uptime !== null
                                                        ? ` (${healthInfo.healthStatus.uptime.toFixed(1)}% uptime)`
                                                        : ''
                                                }`}
                                            />
                                        )
                                ...
                                <span
                                    className={`hidden md:inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium ${
                                    ...
                                    >
                                        <div className="flex items-center justify-between flex-wrap gap-1">
                                            <div className="flex items-center space-x-1 sm:space-x-2 flex-wrap">
                                                ...
                                                {/* Verifiability badge */}
                                                <span
                                                    className={`inline-flex items-center px-1 sm:px-1.5 py-0.5 rounded text-xs font-medium ${
                                ...
                                {filteredProviders.length === 0 ? (
                                    ...
                                    filteredProviders.map((provider) => (
                                        <MobileProviderCard
                                            ...
                                            isRecentlyUsed={recentlyUsedSet.has(
                                                provider.address
                                            )}
                                            onSelect={() =>
                                                handleMobileSelect(provider)
                                            }
                    ...
                    <div className="hidden xl:flex items-center gap-2 flex-1">
                        ...
                        <div className="flex items-center gap-1">
                            <span className="text-xs text-gray-500 font-mono">
                                {selectedProvider.address.slice(0, 6)}...
                                {selectedProvider.address.slice(-4)}
                            </span>
                        ...
                        <div className="hidden lg:block relative group">
                            <div className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-white border border-gray-200">
                                ...
                                <span className="text-xs font-medium text-gray-700">
                                    {selectedProvider.inputPrice !==
                                        undefined && (
                                        <span>
                                            {selectedProvider.inputPrice.toFixed(
                                                2
                                            )}
                                        </span>
                                    )}
                                    ...
                                    {selectedProvider.outputPrice !==
                                        undefined && (
                                        <span>
                                            {selectedProvider.outputPrice.toFixed(
                                                2
                                            )}
                                        </span>
                                    )}
                            ...
                            <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-2 px-3 py-2 bg-gray-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-50 whitespace-nowrap">
                                ...
                                {selectedProvider.inputPrice !== undefined && (
                                    <div>
                                        Input (what you send):{' '}
                                        {selectedProvider.inputPrice.toFixed(4)}{' '}
                                        0G
                                    </div>
                                )}
                                {selectedProvider.outputPrice !== undefined && (
                                ...
                                <div className="text-gray-400 mt-1 border-t border-gray-700 pt-1">
                                    ~
                                    {(
                                        (selectedProvider.inputPrice || 0) +
                                        (selectedProvider.outputPrice || 0)
                                    ).toFixed(4)}{' '}
                        ...
                        {!isConnected ? (
                            ...
                            className={`flex items-center gap-1 sm:gap-1.5 px-1.5 sm:px-2 py-1 rounded-md text-xs ${
                                ...
                                (providerBalance ?? 0) === 0
                                    ...
                                    : providerBalanceNeuron !== null &&
                                      selectedProvider.inputPriceNeuron !==
                                          undefined &&
                                      selectedProvider.outputPriceNeuron !==
                                          undefined &&
                                      providerBalanceNeuron <=
                                          BigInt(50000) *
                                              (selectedProvider.inputPriceNeuron +
                                ...
                                {providerBalance !== null ? (
                                    <>
                                        ...
                                        <span className="hidden sm:inline">
                                            {formatNumber(providerBalance)}
                                        </span>
                                        <span className="ml-0.5">0G</span>
                                        {providerPendingRefund !== null &&
                                            providerPendingRefund > 0 && (
