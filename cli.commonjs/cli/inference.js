...
async function promptDurationSelection() {
    ...
    if (durationChoice === 'custom') {
        console.log()
        const customSeconds = await (0, interactive_selection_1.textInput)(
...
function inference(program) {
    ...
    program
        ...
        .action(async (options) => {
            ...
            const key = await (0,
            private_key_setup_1.ensurePrivateKeyConfiguration)()
            const { runInferenceServer } = await Promise.resolve().then(() =>
                tslib_1.__importStar(require('../example/inference-server'))
            )
    ...
    program
        ...
        .action(async (options) => {
            ...
            const { runRouterServer } = await Promise.resolve().then(() =>
                tslib_1.__importStar(require('../example/router-server'))
            )
    ...
    program
        ...
        .action(async (options) => {
            try {
                const rpcEndpoint = await (0, network_setup_1.getRpcEndpoint)(
                    options
                )
        ...
        .action(async (options) => {
            try {
                ...
                try {
                    ...
                    console.log(
                        `\n${chalk_1.default.blue('Log file:')} ${
                            options.component
                        }/${options.filename}`
                    )
                    console.log(
                        `${chalk_1.default.blue('Provider:')} ${userAddress}`
                    )
            ...
            try {
                ...
                ;(0, util_1.withBroker)(options, async (broker) => {
                    ...
                    try {
                        ...
                        console.log(
                            chalk_1.default.gray(`Service Type: ${serviceType}`)
                        )
                        console.log(
                            chalk_1.default.gray(
                        ...
                        console.log(
                            chalk_1.default.blue('Example curl command:')
                        )
                        console.log()
                        if (serviceType === 'speech-to-text') {
                            console.log(
                                chalk_1.default
                                    .white(`curl ${serviceUrl}/v1/proxy/audio/transcriptions \\
  ...
  -F "response_format=json"`)
                            ...
                            console.log(
                                chalk_1.default
                                    .white(`curl ${serviceUrl}/v1/proxy/images/generations \\
  ...
  -d '{
    ...
    "size": "512x512"
                            ...
                            console.log(
                                chalk_1.default
                                    .white(`curl ${serviceUrl}/v1/proxy/chat/completions \\
  -H "Content-Type: application/json" \\
  -H "Authorization: Bearer ${bearerToken}" \\
  -d '{
    "model": "${serviceModel}",
